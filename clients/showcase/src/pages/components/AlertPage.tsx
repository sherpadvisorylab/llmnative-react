...
const PROPS_CONFIG: PropDef[] = [
    ...
    { name: 'before', type: 'ReactNode', description: 'Content rendered before the alert', control: 'text' },
    { name: 'after', type: 'ReactNode', description: 'Content rendered after the alert', control: 'text' },
];
...
<Alert variant="danger">A critical error occurred</Alert>`}
            />
                    ...
                    <div className="flex flex-col gap-3 w-full">
                        <div className="flex flex-col gap-2">
                            {(['success', 'danger', 'warning', 'info'] as const).map(t => (
                                <Alert key={t} variant={t} appearance="text">{t.charAt(0).toUpperCase() + t.slice(1)} - text appearance</Alert>
                            ))}
                        </div>
                        <div className="flex items-center gap-3 mt-2">
                            <Alert variant="success" appearance="text">Saved</Alert>
                            <button className="btn btn-primary">Save</button>
                        </div>
                    </div>
                }
                code={`// Compact inline status - next to a save button
<Alert variant="success" appearance="text" timeout={3000} onClose={() => setVisible(false)}>
    Saved
</Alert>
<button className="btn btn-primary">Save</button>`}
            />
            ...
            <Section
                ...
                preview={
                    <div className="flex flex-col gap-2 w-full">
                        {visible ? (
                            <Alert variant="success" timeout={3000} onClose={() => setVisible(false)}>
...
<Alert variant="info">Inline alert</Alert>

// Viewport-pinned above everything - ideal for global toasts
<Alert variant="success" placement="fixed" onClose={() => setVisible(false)}>
    Saved successfully
</Alert>`}
            />
