...
| `ai` | Prompt execution and model orchestration | `Prompt`, `WorkflowAI`, `AI.fetch(...)`, `useAIProvider` | `openai`, `openrouter`, `opencode`, `openai-compatible`, `deepseek`, `gemini`, `anthropic`, `mistral`, `glm`, `cloudflare`, `groq` |
...
interface AppProvidersConfig {
  ...
  services?: {
    ...
    email?: 'gmail' | string;
    ai?: 'openai' | 'openrouter' | 'opencode' | 'openai-compatible' | 'deepseek' | 'gemini' | 'anthropic' | 'mistral' | 'glm' | 'cloudflare' | 'groq' | string;
...
interface AIConfig {
  ...
  cloudflare?: {               // Cloudflare Workers AI — needs both apiToken and accountId
    apiToken?: string;
    accountId?: string;
    defaultModel?: string;     // default: '@cf/meta/llama-3.3-70b-instruct-fp8-fast'
    includePaidModels?: boolean; // keep Workers Paid-only models in discovery (default: false)
  };
  openAICompatible?: {
    apiKey?: string;
    baseUrl?: string;
