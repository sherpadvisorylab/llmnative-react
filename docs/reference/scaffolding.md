...
Every generated file is yours to edit freely â€” the scaffold never overwrites after the initial generation.
...
The scaffold asks six questions. Every choice can be changed after generation â€” nothing is locked in.

| Prompt | Options | Effect |
|--------|---------|--------|
| Project name | any string | Sets the folder name and `package.json` name |
| Data provider | `firebase` Â· `supabase` Â· `mock` Â· `custom` | Wires the correct `DataProviderAdapter`; generates `.env` stubs for credentials |
| Icon provider | `lucide` Â· `phosphor` | Sets `iconProvider` in `conf/app.ts` |
| Theme | `default` · `flat` · `cyber` | Sets the built-in self-contained theme passed as `themeProvider` to `<App>` |
| App template | `blank` Â· `crm` Â· `admin` Â· `inventory` Â· `project` | Copies starter pages and mock data for that use case |
...
- **Theme** â€” controls only colors, radius and font. The three built-in options (`default`, `flat`, `cyber`) are full light + dark palettes. Change theme without touching a single page file.
- **Template** â€” controls the initial page set, mock data and navigation. It has no effect on colors. Change pages without touching the theme.
...
my-app/
  ...
  vite.config.ts
  package.json
  .env                       â† VITE_* provider credentials (never commit this)
  src/
    index.tsx                â† Vite entry: mounts <App>
    conf/
      app.ts                 â† provider, theme, icon provider
      menu.ts                â† navigation tree (template-specific)
    layouts/
      Default.tsx            â† shell: header + sidebar + main + footer
    sections/
      Header.tsx             â† topbar: Brand, Menu, Notifications, AuthButton
      Sidebar.tsx            â† responsive sidebar using <Menu context="sidebar">
      Footer.tsx
      PageHeader.tsx         â† Breadcrumbs above page content
      PreLoader.tsx          â† initial spinner, auto-removes after mount
    pages/                   â† starter pages (varies by template)
    data/
      mockData.ts            â† fixture data used by MockDataProvider
    styles/
      globals.css            â† Tailwind v4 entry + --rf-* â†’ color utility bridge
```
...
|----------|----------------|
| `blank` | `home/HomePage` â€” Grid with task list |
| `crm` | `home/HomePage` Â· `contacts/ContactsPage` Â· `companies/CompaniesPage` Â· `deals/DealsPage` |
| `admin` | `home/HomePage` Â· `users/UsersPage` Â· `roles/RolesPage` Â· `settings/SettingsPage` |
| `inventory` | `home/HomePage` Â· `products/ProductsPage` Â· `categories/CategoriesPage` |
| `project` | `home/HomePage` Â· `projects/ProjectsPage` Â· `tasks/TasksPage` Â· `team/TeamPage` |
...
Switch the provider at any time by editing `VITE_PROVIDER` in `.env` â€” no code change needed.
...
```ts
// src/conf/menu.ts  â€” example from the crm template
...
The shell component wired to every route. It renders the Header, Sidebar, main content area and Footer. Edit freely â€” the scaffold generates it as a plain React component with no hidden magic.
...
## `styles/globals.css` â€” Tailwind v4 bridge
