...
| CR-081 | **Implementato, gate verdi — publish sospeso** | `useFormContext` — sottoscrizione selettiva per-path via `useSyncExternalStore` (elimina il re-render di ogni consumer del form ad ogni keystroke su qualunque campo), opt-in `subscribeToFullRecord` per i consumer cross-field (`Prompt.tsx`, e nel consumer CMS `AreaEditorPage.tsx`/`PageEditorPage.tsx`). `tsc`/`test`(704/704)/`build`/`pack` tutti verdi. Issue #25 aperta — resta solo la pubblicazione npm, su richiesta esplicita. |
| CR-037 | ⬜ | Component Builder System — `useImage()` pattern non ancora standardizzato. |
