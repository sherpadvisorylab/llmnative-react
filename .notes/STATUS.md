...
| CR-080 | Done | `Component.input` — aggiunte le chiavi `richtext`/`range`/`url` (componenti già esportati dal framework ma assenti dalla dispatch table). Nessuna logica nuova, stesso pattern delle entry esistenti. Issue #24 chiusa. |
| CR-081 | Done | `useFormContext` — sottoscrizione selettiva per-path via `useSyncExternalStore` (un consumer si ri-renderizza solo quando cambia il proprio campo, non più ad ogni keystroke ovunque nel form), opt-in esplicito `subscribeToFullRecord` per i consumer cross-field (`Prompt.tsx`). Fix minore incluso: `CodeEditor.onUpdate` spostato in un ref, evita il remount della `EditorView` ad ogni save. Issue #25 chiusa. |
| CR-082 | Done | `Gallery` — nuovo prop `renderItem`: sostituisce interamente il contenuto di default di un item (`<img>` + `overlays`) con markup custom, per una card senza immagine (icona/testo/badge). Selezione/click-handling/sizing griglia restano gestiti da `Gallery`. Threading completo `Grid`/`GridCore` → `GridGalleryView` → `Gallery` via `views.gallery.renderItem`. 8 nuovi test (5 Gallery, 3 Grid — prima copertura end-to-end di `view="gallery"` da `Grid`, gap pre-esistente). Showcase aggiornato: nuova Section "Custom card (no image)" + playground toggle su GalleryPage, nuovo tab "Gallery (custom card)" su GridPage, i18n 6 lingue. Issue #26 chiusa. |
...
src/
  ...
  providers/
    ...
    ai/                    # AI multi-provider (OpenAI/Gemini/Anthropic/DeepSeek/Mistral/…/GLM/Cloudflare)
...
| `npm test` | Passes: 66 files, 744 tests. |
| `npm run build` | Passes: Vite library build + declarations. |
| `npm pack --dry-run --json` | Passes: 218 entries. |
| Live OpenCode Zen | `/zen/v1/models` no longer lists `deepseek-v4-flash-free`; every free-tier model (`big-pickle`, `*-free`) answers "OpenCode's free tier can only be used from within OpenCode". |
...
|---------|--------|
| `npx tsc --noEmit` | Passes: 0 errors. |
| `npm test` | Passes: 65 files, 730 tests. |
| `npm run build` | Passes: Vite library build + declarations. |
| `npm pack --dry-run --json` | Passes: 217 entries. |
| Live Workers AI (Free plan, real token) | validateApiKey ok / wrong account / bad token; 24-model catalog; text; 2-turn tool calling on llama-3.3-70b, gpt-oss-120b, qwen3-30b; vision on llama-4-scout; readable 5035 error on a Workers Paid model. |
...
|---------|--------|
| `npx tsc --noEmit` | Passes: 0 errors. |
| `npm test` | Passes: 64 files, 705 tests. |
| `npm run build` | Passes: Vite library build + declarations. |
| `npm pack --dry-run --json` | Passes: 216 entries. |
...
| 1.10.0 | Published on npm (`@llmnative/react@1.10.0`). CR-081 (`useFormContext` selective per-path subscription + `subscribeToFullRecord` opt-in) completa. Minor fix (no CR): `CodeEditor`'s `onUpdate` moved off the CodeMirror mount effect's deps into a ref, avoiding an `EditorView` remount on every form save. |
| 1.10.1 | Published on npm (`@llmnative/react@1.10.1`). Minor fix (no CR): `Prompt` (RUN mode) no longer falls back to the provider's hardcoded `defaultModel` when no model is explicitly selected — a stored/stale `localStorage['prompt.model']` no longer in the live catalog now leaves the selection empty and Run disabled instead of silently resolving to a hand-written default that goes stale as fast as the provider's real model list changes. |
| 1.10.2 | Published on npm (`@llmnative/react@1.10.2`). Minor fix (no CR): `getAIModelCatalog` isolates each provider's `getCapabilities()` in its own try/catch instead of one unguarded `Promise.all` — one provider rejecting no longer silently blanks every provider's models (consumer's `.catch` had nothing to log; found via a real-world CMS repro with 2 tenant providers configured, model dropdown vanished entirely with zero console errors). New regression test in `AIProviderRegistry.test.ts`. |
| 1.10.3 | Published on npm (`@llmnative/react@1.10.3`). Minor addition (no CR): `Chatbot` gained an optional `modelPlaceholder` prop — lets a consumer that never auto-selects a model show a clearer trigger label ("Choose a model…") than the shared generic "Default" used by the role/language/voice/style selectors. |
| 1.11.0 | Published on npm (`@llmnative/react@1.11.0`). Minor addition (no CR): `SideNav` gained optional controlled-collapse props `collapsed`/`onCollapsedChange` (standard controlled/uncontrolled pattern) — lets a consumer drive the collapsed state itself (e.g. auto-collapse to icon-only when its own layout detects the content area got too narrow), instead of only the component's own internal state + footer toggle. Fully backward compatible: omitting both props keeps prior behavior unchanged. |
