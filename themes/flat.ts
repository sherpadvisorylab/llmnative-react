import type { Theme, ThemeDefinition, ThemePresetConfig } from '../src/Theme';
import type { MotionRegistry } from '../src/motion';

export const preset: ThemePresetConfig = {
    mode: 'light' as const,
    radius: 0.125,
    colors: {
        background:           '0 0% 98%',
        foreground:           '215 25% 16%',
        card:                 '0 0% 100%',
        cardForeground:       '215 25% 16%',
        popover:              '0 0% 100%',
        popoverForeground:    '215 25% 16%',
        primary:              '215 25% 27%',
        primaryForeground:    '210 40% 98%',
        secondary:            '215 15% 93%',
        secondaryForeground:  '215 25% 27%',
        muted:                '215 15% 93%',
        mutedForeground:      '215 15% 50%',
        accent:               '215 15% 90%',
        accentForeground:     '215 25% 27%',
        destructive:          '0 72% 51%',
        destructiveForeground:'0 0% 100%',
        success:              '142.1 76.2% 36.3%',
        successForeground:    '0 0% 100%',
        warning:              '32.1 94.6% 43.7%',
        warningForeground:    '0 0% 100%',
        info:                 '198.6 88.7% 48.4%',
        infoForeground:       '0 0% 100%',
        border:               '215 15% 85%',
        input:                '215 15% 85%',
        ring:                 '215 25% 27%',
    },
    dark: {
        background:           '215 20% 12%',
        foreground:           '215 10% 85%',
        card:                 '215 20% 15%',
        cardForeground:       '215 10% 85%',
        popover:              '215 20% 15%',
        popoverForeground:    '215 10% 85%',
        primary:              '215 35% 55%',
        primaryForeground:    '0 0% 100%',
        secondary:            '215 20% 20%',
        secondaryForeground:  '215 10% 75%',
        muted:                '215 20% 20%',
        mutedForeground:      '215 10% 55%',
        accent:               '215 20% 22%',
        accentForeground:     '215 10% 85%',
        destructive:          '0 62% 40%',
        destructiveForeground:'0 0% 100%',
        success:              '142.1 70.6% 45.3%',
        successForeground:    '0 0% 100%',
        warning:              '32.1 94.6% 55%',
        warningForeground:    '0 0% 100%',
        info:                 '198.6 88.7% 60%',
        infoForeground:       '0 0% 100%',
        border:               '215 20% 22%',
        input:                '215 20% 22%',
        ring:                 '215 35% 55%',
    },
};

export const motion: MotionRegistry = {
    none: {
        from: {},
        to: {},
        transition: { duration: 0, easing: 'linear', properties: ['opacity', 'transform', 'box-shadow'] },
        reducedMotion: 'always',
    },
    fade: {
        from: { opacity: 0 },
        to: { opacity: 1 },
        transition: { duration: 140, easing: 'cubic-bezier(0.2, 0, 0, 1)', properties: ['opacity'] },
        reducedMotion: 'respect-user',
    },
    fadeUp: {
        from: { opacity: 0, transform: 'translateY(4px)' },
        to: { opacity: 1, transform: 'translateY(0)' },
        transition: { duration: 140, easing: 'cubic-bezier(0.2, 0, 0, 1)', properties: ['opacity', 'transform'] },
        reducedMotion: 'respect-user',
    },
    fadeDown: {
        from: { opacity: 0, transform: 'translateY(-4px)' },
        to: { opacity: 1, transform: 'translateY(0)' },
        transition: { duration: 120, easing: 'cubic-bezier(0.2, 0, 0, 1)', properties: ['opacity', 'transform', 'visibility'] },
        reducedMotion: 'respect-user',
    },
    slideFromRight: {
        from: { opacity: 0, transform: 'translateX(100%)' },
        to: { opacity: 1, transform: 'translateX(0)' },
        transition: { duration: 180, easing: 'cubic-bezier(0.16, 1, 0.3, 1)', properties: ['opacity', 'transform'] },
        reducedMotion: 'respect-user',
    },
    slideFromLeft: {
        from: { opacity: 0, transform: 'translateX(-100%)' },
        to: { opacity: 1, transform: 'translateX(0)' },
        transition: { duration: 180, easing: 'cubic-bezier(0.16, 1, 0.3, 1)', properties: ['opacity', 'transform'] },
        reducedMotion: 'respect-user',
    },
    slideFromTop: {
        from: { opacity: 0, transform: 'translateY(-100%)' },
        to: { opacity: 1, transform: 'translateY(0)' },
        transition: { duration: 180, easing: 'cubic-bezier(0.16, 1, 0.3, 1)', properties: ['opacity', 'transform'] },
        reducedMotion: 'respect-user',
    },
    slideFromBottom: {
        from: { opacity: 0, transform: 'translateY(100%)' },
        to: { opacity: 1, transform: 'translateY(0)' },
        transition: { duration: 180, easing: 'cubic-bezier(0.16, 1, 0.3, 1)', properties: ['opacity', 'transform'] },
        reducedMotion: 'respect-user',
    },
    press: {
        from: {},
        to: { transform: 'scale(0.99)' },
        transition: { duration: 90, easing: 'cubic-bezier(0.2, 0, 0, 1)', properties: ['transform', 'box-shadow'] },
        reducedMotion: 'respect-user',
    },
    imageZoom: {
        from: { transform: 'scale(1)' },
        to: { transform: 'scale(1.06)' },
        transition: { duration: 300, easing: 'cubic-bezier(0.2, 0, 0, 1)', properties: ['transform'] },
        reducedMotion: 'respect-user',
    },
};

export const components: Theme = {
    Grid: {
        // REVERT NOTE (2026-07-15): see themes/default.ts for the full rationale/before-state.
        Card: {
            wrapperClassName: '',
            className: 'rounded-none border-0 bg-transparent shadow-none',
            headerClassName: 'flex justify-between border-b-0 bg-transparent px-0 py-2',
            bodyClassName: 'p-0 mt-2',
            footerClassName: 'border-t-0 bg-transparent px-0 py-2',
            loading: false,
        },
        Table: {
            wrapperClassName: 'rounded-lg border border-border bg-card shadow-sm',
            className: '',
            headerClassName: '',
            bodyClassName: '',
            footerClassName: '',
            scrollClassName: 'overflow-x-auto',
            selectedClassName: 'bg-primary/10',
        },
        Gallery: {
            wrapperClassName: 'rounded-lg border border-border bg-card shadow-sm p-3',
            className: '',
            scrollClassName: '',
            headerClassName: '',
            bodyClassName: '',
            footerClassName: '',
            selectedClassName: 'bg-primary/20',
            gap: 1,
            rowCols: 4,
        },
        Modal: {
            mode: 'form',
            size: 'lg',
            position: 'center',
            wrapperClassName: '',
            className: '',
            headerClassName: '',
            titleClassName: '',
            subtitleClassName: 'pr-1 text-muted-foreground',
            bodyClassName: '',
            footerClassName: '',
            iconExpand: 'fullscreen',
            iconCollapse: 'fullscreen-exit',
            motion: {
                center: 'fade',
                top: 'slideFromTop',
                left: 'slideFromLeft',
                right: 'slideFromRight',
                bottom: 'slideFromBottom',
                backdrop: 'fade',
            },
        },
    },
    Table: {
        wrapperClassName: '',
        className: '',
        headerClassName: '',
        bodyClassName: '',
        footerClassName: '',
        scrollClassName: 'overflow-x-auto',
        selectedClassName: 'bg-primary/10',
    },
    Gallery: {
        wrapperClassName: '',
        className: '',
        scrollClassName: '',
        headerClassName: '',
        bodyClassName: '',
        footerClassName: '',
        selectedClassName: 'bg-primary/20',
        gap: 4,
        rowCols: 2,
    },
    Pagination: {
        wrapperClassName: '',
        className: '',
        stickyClassName: 'fixed bottom-0 left-0 right-0 mx-5',
        scrollToTop: false,
        scrollBehavior: 'auto',
        maxItems: 5,
        sticky: true,
        align: 'end',
    },
    Carousel: {
        showIndicators: true,
        showControls: true,
        showCaption: true,
        dark: false,
        autoPlay: true,
    },
    Card: {
        wrapperClassName: '',
        className: '',
        headerClassName: 'flex justify-between bg-white/[.15] font-normal',
        bodyClassName: 'flex flex-col',
        footerClassName: '',
        loading: false,
    },
    Loader: {
        wrapperClassName: '',
        className: '',
        icon: 'loader-2',
        title: 'Loading..',
        description: '',
    },
    ActionButton: {
        className: 'btn-primary',
        badgeClassName: 'rounded-full bg-destructive text-destructive-foreground',
        motion: {
            press: 'press',
        },
    },
    LoadingButton: {
        className: 'btn-primary',
        badgeClassName: 'rounded-full bg-destructive text-destructive-foreground',
        motion: {
            press: 'press',
        },
    },
    LinkButton: {
        className: 'btn-outline-secondary',
    },
    Alert: {
        className: '',
    },
    Badge: {
        className: '',
    },
    Modal: {
        size: 'lg',
        position: 'center',
        wrapperClassName: '',
        className: '',
        headerClassName: '',
        titleClassName: '',
        subtitleClassName: 'pr-1 text-muted-foreground',
        bodyClassName: '',
        footerClassName: '',
        iconExpand: 'fullscreen',
        iconCollapse: 'fullscreen-exit',
        motion: {
            center: 'fade',
            top: 'slideFromTop',
            left: 'slideFromLeft',
            right: 'slideFromRight',
            bottom: 'slideFromBottom',
            backdrop: 'fade',
        },
    },
    Dropdown: {
        wrapperClassName: '',
        className: '',
        triggerClassName: 'btn btn-outline-primary',
        badgeClassName: '',
        menuClassName: 'border-border/60 bg-popover/95 shadow-lg shadow-black/5 supports-[backdrop-filter]:bg-popover/92',
        menuHeaderClass: 'text-muted-foreground/70',
        menuItemClass: '',
        menuDividerClass: 'bg-border/60',
        headerClassName: '',
        footerClassName: '',
        Menu: {
            wrapperClassName: '',
            className: 'list-none p-0 m-0',
            headerClassName: '',
            itemClassName: '',
            linkClassName: 'flex items-center gap-2 rounded-sm px-2 py-1.5 text-sm hover:bg-accent',
            iconClassName: 'text-muted-foreground',
            textClassName: '',
            badgeClassName: '',
            arrowClassName: '',
            submenuClassName: '',
        },
        motion: {
            open: 'fadeDown',
            close: 'fadeDown',
            press: 'press',
        },
    },
    Notifications: {
        wrapperClassName: '',
        Dropdown: {
            className: '',
            triggerClassName: 'btn btn-link',
            menuClassName: 'mt-1 text-[11px] w-[300px] pt-1',
        },
    },
    Select: {
        wrapperClassName: '',
        className: '',
    },
    Autocomplete: {
        wrapperClassName: '',
        className: '',
    },
    Form: {
        wrapperClassName: '',
        buttonSaveClass: 'btn-outline-primary',
        buttonDeleteClass: 'btn-outline-danger',
        buttonBackClass: 'btn-link',
        Card: {
            headerClassName: '',
            bodyClassName: '',
            footerClassName: 'flex items-center justify-end gap-3',
        },
    },
    Menu: {
        wrapperClassName: 'flex-1 overflow-auto p-4',
        className: 'flex flex-col mb-auto',
        headerClassName: '',
        itemClassName: '',
        linkClassName: 'flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium text-muted-foreground transition-colors hover:bg-accent/50 hover:text-foreground',
        iconClassName: 'mr-1 shrink-0',
        textClassName: 'flex-1',
        badgeClassName: 'ml-1',
        arrowClassName: '',
        submenuClassName: 'mt-1 flex flex-col gap-1 pl-4',
    },
    SideNav: {
        wrapperClassName: '',
        shellClassName: 'border-r border-border bg-card text-foreground',
        overlayClassName: 'shadow-lg',
        headerClassName: 'border-b border-border px-1.5 py-2',
        navClassName: 'px-1.5 py-2',
        footerClassName: 'border-t border-border px-1.5 py-2',
        groupLabelClassName: 'text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground/70 select-none',
        groupDividerClassName: 'bg-border/60',
        itemClassName: 'rounded-md',
        itemActiveClassName: 'bg-primary/10 text-primary font-medium',
        itemChildActiveClassName: 'bg-accent text-foreground font-medium',
        itemInactiveClassName: 'text-muted-foreground hover:text-foreground hover:bg-accent/80',
        itemIconClassName: '',
        itemTextClassName: '',
        itemBadgeClassName: 'ml-auto shrink-0 text-[10px] font-semibold leading-none px-1.5 py-[3px] rounded-full bg-primary/15 text-primary',
        itemCollapsedBadgeClassName: 'absolute -top-0.5 -right-1 h-[7px] w-[7px] rounded-full bg-primary ring-1 ring-card',
        itemToggleClassName: 'shrink-0 flex items-center justify-center w-6 h-6 rounded transition-colors text-muted-foreground/50 hover:text-foreground hover:bg-accent/80',
        subItemClassName: 'rounded-sm',
        subItemActiveClassName: 'bg-primary/10 text-primary font-medium',
        subItemInactiveClassName: 'text-muted-foreground hover:text-foreground hover:bg-accent/80',
        collapseButtonClassName: 'shrink-0 flex items-center justify-center rounded-md w-8 h-8 transition-colors text-muted-foreground hover:text-foreground hover:bg-accent/80',
    },
    Toolbar: {
        wrapperClassName: 'z-20 w-full',
        stickyClassName: 'sticky top-0',
        restingClassName: 'bg-transparent border-transparent shadow-none',
        scrolledClassName: 'border-b border-border bg-background/90 backdrop-blur supports-[backdrop-filter]:bg-background/85',
        innerClassName: 'flex min-h-13 items-center gap-3 px-4 md:px-5',
        leadingClassName: 'flex min-w-0 items-center gap-2 shrink-0',
        centerClassName: 'flex min-w-0 flex-1 items-center gap-3',
        trailingClassName: 'flex items-center gap-2 shrink-0',
    },
    ListCard: {
        wrapperClassName: '',
        className: '',
        activeClassName: 'border-primary/30 bg-primary/10',
        inactiveClassName: 'hover:bg-accent/60',
        disabledClassName: 'cursor-not-allowed opacity-60',
        dashedClassName: 'border-dashed',
        compactClassName: 'p-3',
        leadingClassName: '',
        bodyClassName: '',
        headerClassName: '',
        titleClassName: '',
        descriptionClassName: '',
        metaClassName: '',
        badgeClassName: '',
        trailingClassName: '',
        footerClassName: '',
        motion: {
            press: 'press',
        },
    },
    ProviderSwitcher: {
        wrapperClassName: 'w-full',
        className: 'w-full',
        triggerClassName: 'flex w-full items-center gap-2 rounded-xl px-1.5 py-1.5 text-left text-sm font-semibold text-foreground transition-colors hover:bg-accent/60',
        disabledClassName: 'pointer-events-none opacity-60',
        iconWrapperClassName: 'flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary',
        labelClassName: 'block truncate',
        captionClassName: 'block truncate text-[11px] font-normal text-muted-foreground/80',
        chevronClassName: 'ml-auto shrink-0 text-muted-foreground/70',
        itemClassName: '',
        itemActiveClassName: '',
        itemInactiveClassName: '',
        itemMetaClassName: 'text-xs text-muted-foreground',
        checkClassName: 'text-primary',
    },
    Brand: {
        wrapperClassName: '',
        className: 'flex items-center',
        logoClassName: 'flex items-center gap-2 font-semibold',
        labelClassName: 'text-sm font-medium',
    },
    SignIn: {
        className: 'flex items-center',
        avatarClass: 'mx-2 rounded-full object-cover',
    },
    Image: {
        wrapperClassName: '',
        className: '',
        motion: {
            enter: 'fade',
            hover: false,
        },
    },
    ImageAvatar: {
        wrapperClassName: '',
        className: '',
        motion: { enter: 'fade', hover: false },
    },
    Percentage: {
        wrapperClassName: '',
        className: '',
    },
    Tab: {
        wrapperClassName: '',
        className: '',
        menuClassName: '',
        itemClassName: '',
        contentClassName: '',
        triggerClassName: '',
        activeTriggerClassName: '',
        inactiveTriggerClassName: '',
        motion: {
            enter: 'fadeUp',
        },
    },
    Code: {
        wrapperClassName: '',
        className: '',
    },
    CodeEditor: {
        wrapperClassName: '',
        className: '',
    },
    Prompt: {
        wrapperClassName: '',
        className: '',
    },
};

const definition: ThemeDefinition = { preset, motion, components };

export default definition;

